...
void kernel_panic(struct registers regs) {
    ...
    panic_center(6, "Sistemde Kritik bir hata tespit edildi.", PANIC_WHITE);

    panic_puts(9,  4, "Hata Kodu : ", PANIC_WHITE);
    panic_puts(9, 16, code,           PANIC_YELLOW);

#ifdef __x86_64__
    panic_puts(11, 4, "Mimari    : x86_64", PANIC_WHITE);
#else
    panic_puts(11, 4, "Mimari    : i386",   PANIC_WHITE);
#endif

    panic_center(19, "Hatanin sebebini bulamiyor musunuz?", PANIC_WHITE);
    panic_center(20, "Yardim icin:", PANIC_WHITE);
    panic_center(21, "github.com/bigpower217th-ui/valiantcore", PANIC_YELLOW);
    panic_center(22, "-> Issues sekmesini kullanin <-", PANIC_WHITE);

    panic_fill_row(24, PANIC_YELLOW);
    panic_center(24, "ValiantCore Kernel — github.com/bigpower", PANIC_YELLOW);
